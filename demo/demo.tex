...
\usetheme[ % choose your options, choose wisely
	...
	titleformat subtitle= allsmallcaps, % regular, smallcaps, allsmallcaps, allcaps
...
%
%%%------------- Old style figures / "lowercase" numbers -------------%
...
\newcommand{\done}{\rlap{$\openbox$}{\raisebox{1.5pt}{\large\hspace{1pt}\cmark}}\hspace{-2.2pt}}
\newcommand{\wontfix}{\rlap{$\openbox$}{\large\hspace{.5pt}\xmark}\hspace{-0pt}}
\makeatletter
...
\usetheme[]{metroazwei}
\end{verbatim}
Take a look into the source code for this demo for different options and take a look at the original documentation of metropolis.
\end{frame}
...
\begin{frame}{Title formats}
	...
	\begin{itemize}
		...
		\item ALL CAPS
	\end{itemize}
	It can be set globally for presentation title \& subtitle and frametitles.
	They can also be set individually for frametitles, see next slide.
\end{frame}
...
\begin{frame}{A slide with a Framesubtitle}
\framesubtitle{but it breaks the design quite a lot}
So maybe avoid using it and rather use a

\heading{Heading}
in your slide, which is expained on slide~\ref{important}: \hyperlink{important}{Headings}

\end{frame}

\begin{frame}[fragile]{Headings}\label{important}
Sometimes you want to talk about different
\heading{Topics}
in one slide or need something to structure it?
\alertheading{Use Headings!}
They will always have the same style throughout your presentation.

\begin{verbatim}
\heading{Topic}
\alertheading{Important Topic}
\end{verbatim}
\end{frame}
...
\subsection{Code Frames}

\begin{frame}[fragile]{Frames with Code}
The package \texttt{listings} is already included in the metroazwei theme, so you can use the \texttt{lstlisting} environment to show off your code:
\begin{lstlisting}
// 'Hello World!' program 
...
\end{lstlisting}
By default it highlights but you can change the settings, just look up the documentation.
\end{frame}

\section{more Features which are only included in this demo.tex}
...
Get the theme at:
\begin{center}\url{github.com/A2-Collaboration/metroazwei}\end{center}
